...
impl SmartMarginContract {
    ...
    pub fn add_session_key(env: Env, user: Address, session_key: Address) {
        ...
        env.storage()
            .persistent()
            .set(&DataKey::SessionKey(user.clone()), &session_key);
        env.storage()
            .persistent()
            .extend_ttl(&DataKey::SessionKey(user), 10_000, 100_000);
    }
    ...
    pub fn remove_session_key(env: Env, user: Address) {
        ...
        env.storage()
            .persistent()
            .remove(&DataKey::SessionKey(user));
    }
    ...
    fn verify_caller(env: &Env, user: &Address, caller: &Address) -> Result<(), Error> {
        if user == caller {
            ...
            let registered_key: Address = env
                .storage()
                .persistent()
    ...
    pub fn deposit_margin(env: Env, user: Address, amount: i128) {
        ...
        let mut bal: i128 = env
            .storage()
            .persistent()
            .get(&DataKey::MarginBalance(user.clone()))
            .unwrap_or(0);
        bal += amount;
        env.storage()
            .persistent()
            .set(&DataKey::MarginBalance(user.clone()), &bal);
        env.storage()
            .persistent()
            .extend_ttl(&DataKey::MarginBalance(user), 10_000, 100_000);
    }
    ...
    pub fn withdraw_margin(
        ...
        let positions: soroban_sdk::Vec<Position> = env
            .storage()
            .persistent()
        ...
        env.storage()
            .persistent()
            .set(&DataKey::MarginBalance(user.clone()), &new_bal);
        env.storage().persistent().extend_ttl(
            &DataKey::MarginBalance(user.clone()),
            10_000,
            100_000,
        );
    ...
    pub fn get_margin_balance(env: Env, user: Address) -> i128 {
        if env
            .storage()
            .persistent()
            .has(&DataKey::MarginBalance(user.clone()))
        {
            env.storage().persistent().extend_ttl(
                &DataKey::MarginBalance(user.clone()),
                10_000,
        ...
        env.storage()
            .persistent()
            .get(&DataKey::MarginBalance(user))
            .unwrap_or(0)
    }
    ...
    fn update_funding(env: &Env) {
        ...
        let last_time: u64 = env
            .storage()
            .instance()
            .get(&DataKey::LastFundingTime)
            .unwrap_or(current_time);

        if current_time > last_time {
            let time_elapsed = (current_time - last_time) as i128;
            let total_long: i128 = env
                .storage()
                .instance()
                .get(&DataKey::TotalLongOI)
                .unwrap_or(0);
            let total_short: i128 = env
                .storage()
                .instance()
                .get(&DataKey::TotalShortOI)
                .unwrap_or(0);
            ...
            let mut global_funding: i128 = env
                .storage()
                .instance()
                .get(&DataKey::GlobalFundingRate)
                .unwrap_or(0);
            global_funding += funding_for_period;

            env.storage()
                .instance()
                .set(&DataKey::GlobalFundingRate, &global_funding);
        }

        env.storage()
            .instance()
            .set(&DataKey::LastFundingTime, &current_time);
    }
    ...
    pub fn add_supported_token(
        ...
        env.storage()
            .instance()
            .set(&DataKey::SupportedTokenSymbol(token.clone()), &symbol);

        let mut tokens: soroban_sdk::Vec<Address> = env
            .storage()
            .instance()
            .get(&DataKey::SupportedTokensArray)
            .unwrap_or(soroban_sdk::Vec::new(&env));
        if !tokens.contains(&token) {
            tokens.push_back(token);
            env.storage()
                .instance()
                .set(&DataKey::SupportedTokensArray, &tokens);
        }
        Ok(())
    }

    pub fn get_aum(env: &Env) -> i128 {
        let tokens: soroban_sdk::Vec<Address> = env
            .storage()
            .instance()
            .get(&DataKey::SupportedTokensArray)
            .unwrap_or(soroban_sdk::Vec::new(env));
        ...
        for token in tokens.iter() {
            ...
            if balance > 0 {
                let symbol: Symbol = env
                    .storage()
                    .instance()
                    .get(&DataKey::SupportedTokenSymbol(token.clone()))
                    .unwrap();
    ...
    pub fn add_liquidity(
        ...
        let symbol: Symbol = env
            .storage()
            .instance()
            .get(&DataKey::SupportedTokenSymbol(token_address.clone()))
            .unwrap();
        let price: i128 = env.invoke_contract(
            &oracle_addr,
            &Symbol::new(&env, "get_price"),
        ...
        let mut lp_bal: i128 = env
            .storage()
            .persistent()
            .get(&DataKey::LpBalance(lp.clone()))
            .unwrap_or(0);
        lp_bal += shares_to_mint;
        env.storage()
            .persistent()
            .set(&DataKey::LpBalance(lp), &lp_bal);

        let mut pool_bal: i128 = env
            .storage()
            .instance()
            .get(&DataKey::PoolBalance(token_address.clone()))
            .unwrap_or(0);
        pool_bal += amount;
        env.storage()
            .instance()
            .set(&DataKey::PoolBalance(token_address), &pool_bal);
    ...
    pub fn remove_liquidity(
        ...
        let total_long_oi: i128 = env
            .storage()
            .instance()
            .get(&DataKey::TotalLongOI)
            .unwrap_or(0);
        let total_short_oi: i128 = env
            .storage()
            .instance()
            .get(&DataKey::TotalShortOI)
            .unwrap_or(0);
        ...
        env.storage()
            .persistent()
            .set(&DataKey::LpBalance(lp.clone()), &lp_bal);
        env.storage()
            .instance()
            .set(&DataKey::PoolBalance(token_address.clone()), &pool_bal);
    ...
    pub fn open_position(
        ...
        if is_long {
            if take_profit > 0 && take_profit <= entry_price {
                return Err(Error::InvalidOrder);
            }
            if stop_loss > 0 && stop_loss >= entry_price {
                return Err(Error::InvalidOrder);
            }
        } else {
            if take_profit > 0 && take_profit >= entry_price {
        ...
        let mut pool_bal: i128 = env
            .storage()
            .instance()
            .get(&DataKey::PoolBalance(token_addr.clone()))
            .unwrap_or(0);
        pool_bal += pool_cut;
        env.storage()
            .instance()
            .set(&DataKey::PoolBalance(token_addr.clone()), &pool_bal);

        let mut ins_bal: i128 = env
            .storage()
            .instance()
        ...
        env.storage()
            .instance()
            .set(&DataKey::InsuranceFundBalance, &ins_bal);

        let pos_id: u64 = env
            .storage()
            .persistent()
            .get(&DataKey::NextPosId(user.clone()))
            .unwrap_or(1);
        ...
        let mut positions: soroban_sdk::Vec<Position> = env
            .storage()
            .persistent()
            .get(&DataKey::Positions(user.clone()))
            .unwrap_or(soroban_sdk::Vec::new(&env));
        ...
        env.storage()
            .persistent()
            .extend_ttl(&DataKey::Positions(user.clone()), 10_000, 100_000);
        env.storage()
            .persistent()
            .set(&DataKey::NextPosId(user.clone()), &(pos_id + 1));
        ...
        if is_long {
            ...
            env.storage()
                .instance()
                .set(&DataKey::TotalLongOI, &total_long);
        } else {
            let mut total_short: i128 = env
                .storage()
                .instance()
                .get(&DataKey::TotalShortOI)
                .unwrap_or(0);
            ...
            env.storage()
                .instance()
                .set(&DataKey::TotalShortOI, &total_short);
        }

        // Task #7: Emits pos_open event
        env.events().publish(
            (Symbol::new(&env, "pos_open"), user),
            (pos_id, margin, leverage, is_long, entry_price),
        );
    ...
    pub fn place_limit_order(
        ...
        let mut orders: soroban_sdk::Vec<Order> = env
            .storage()
            .persistent()
            .get(&DataKey::LimitOrders(user.clone()))
            .unwrap_or(soroban_sdk::Vec::new(&env));
        orders.push_back(order);
        env.storage()
            .persistent()
            .set(&DataKey::LimitOrders(user.clone()), &orders);
        env.storage()
            .persistent()
            .extend_ttl(&DataKey::LimitOrders(user.clone()), 10_000, 100_000);

        // Event #7: OrderPlaced
        env.events().publish(
            (Symbol::new(&env, "order_place"), user),
            (trigger_price, is_long, margin, leverage),
        );
    ...
    pub fn cancel_limit_order(
        ...
        env.events()
            .publish((Symbol::new(&env, "order_cancel"), user), index);
        Ok(())
    }

    pub fn execute_limit_orders(env: Env, user: Address) -> Result<u32, Error> {
        let orders: soroban_sdk::Vec<Order> = env
            .storage()
            .persistent()
            .get(&DataKey::LimitOrders(user.clone()))
            .unwrap_or(soroban_sdk::Vec::new(&env));
        ...
        for order in orders.iter() {
            ...
            if triggered {
                ...
                if user_bal >= order.margin + open_fee {
                    ...
                    env.storage()
                        .persistent()
                        .set(&DataKey::MarginBalance(user.clone()), &user_bal);
        ...
        env.storage()
            .persistent()
            .set(&DataKey::LimitOrders(user), &remaining_orders);
        Ok(executed_count)
    }
    ...
    pub fn modify_tpsl(
        ...
        env.storage()
            .persistent()
            .set(&DataKey::Positions(user.clone()), &updated_positions);
        env.storage()
            .persistent()
            .extend_ttl(&DataKey::Positions(user.clone()), 10_000, 100_000);

        env.events().publish(
            (Symbol::new(&env, "tpsl_update"), user),
            (position_id, take_profit, stop_loss, trailing_stop_distance),
        );
        Ok(())
    }

    pub fn update_trailing_stop(env: Env, user: Address) -> Result<(), Error> {
        let positions_opt: Option<soroban_sdk::Vec<Position>> = env
            .storage()
            .persistent()
            .get(&DataKey::Positions(user.clone()));
        ...
        if updated_any {
            env.storage()
                .persistent()
                .set(&DataKey::Positions(user), &updated_positions);
        }
        Ok(())
    }

    fn internal_close_position(
        env: &Env,
        user: &Address,
        position_id: u64,
        margin_to_close: i128,
        keeper_opt: Option<&Address>,
    ) -> Result<i128, Error> {
        let positions_opt: Option<soroban_sdk::Vec<Position>> = env
            .storage()
            .persistent()
        ...
        let pnl = (price_diff * actual_margin_to_close * (position.leverage as i128))
        ...
        if let Some(keeper) = keeper_opt {
            if keeper != user {
                ...
                if bal >= keeper_reward {
                    bal -= keeper_reward;
                    let mut keeper_bal: i128 = env
                        .storage()
                        .persistent()
                        .get(&DataKey::MarginBalance(keeper.clone()))
                        .unwrap_or(0);
                    keeper_bal += keeper_reward;
                    env.storage()
                        .persistent()
                        .set(&DataKey::MarginBalance(keeper.clone()), &keeper_bal);
                    env.storage().persistent().extend_ttl(
                        &DataKey::MarginBalance(keeper.clone()),
                        10_000,
                        100_000,
                    );
                    ...
                    env.events().publish(
                        (Symbol::new(env, "liquidate"), user.clone()),
                        (keeper.clone(), keeper_reward),
                    );
                }
            }
        }
        ...
        if position.is_long {
            ...
            env.storage()
                .instance()
                .set(&DataKey::TotalLongOI, &updated_long);
        } else {
            let total_short: i128 = env
                .storage()
                .instance()
            ...
            env.storage()
                .instance()
                .set(&DataKey::TotalShortOI, &updated_short);
        }

        let mut total_pnl: i128 = env
            .storage()
            .persistent()
            .get(&DataKey::UserTotalPnL(user.clone()))
            .unwrap_or(0);
        total_pnl += actual_trader_pnl;
        env.storage()
            .persistent()
            .set(&DataKey::UserTotalPnL(user.clone()), &total_pnl);
        env.storage().persistent().extend_ttl(
            &DataKey::UserTotalPnL(user.clone()),
            10_000,
            100_000,
        );
        ...
        if let Some(referrer) = env
            .storage()
            .persistent()
            .get::<DataKey, Address>(&DataKey::ReferralOf(user.clone()))
        {
            let kickback = pool_fee_cut / 5; // 20% of the fee going to pool
            let mut ref_kickback: i128 = env
                .storage()
                .persistent()
                .get(&DataKey::ReferralKickback(referrer.clone()))
                .unwrap_or(0);
            ref_kickback += kickback;
            env.storage()
                .persistent()
                .set(&DataKey::ReferralKickback(referrer.clone()), &ref_kickback);
            env.storage().persistent().extend_ttl(
                &DataKey::ReferralKickback(referrer.clone()),
                10_000,
                100_000,
            );
            let mut ref_lifetime: i128 = env
                .storage()
                .persistent()
                .get(&DataKey::ReferralLifetime(referrer.clone()))
                .unwrap_or(0);
            ref_lifetime += kickback;
            env.storage()
                .persistent()
                .set(&DataKey::ReferralLifetime(referrer.clone()), &ref_lifetime);
            env.storage().persistent().extend_ttl(
                &DataKey::ReferralLifetime(referrer.clone()),
                10_000,
                100_000,
            );
        }

        // Task #7: Emits pos_close event with symbol and exit price for enhanced frontend history display
        env.events().publish(
            (Symbol::new(env, "pos_close"), user.clone()),
            (
                position_id,
                actual_margin_to_close,
                actual_trader_pnl,
    ...
    pub fn trigger_orders(env: Env, caller: Address, user: Address) -> Result<i128, Error> {
        ...
        for position in positions.iter() {
            ...
            // 1. TP / SL Checks
            if position.is_long {
                if position.take_profit > 0 && current_price >= position.take_profit {
                    should_trigger = true;
                }
            ...
            if !should_trigger {
                ...
                let pnl = (price_diff * position.margin * (position.leverage as i128))
                    / position.entry_price;
        ...
        if let Some(pos_id) = triggered_id {
            caller.require_auth();
            return Self::internal_close_position(
                &env,
                &user,
                pos_id,
                triggered_margin,
    ...
    pub fn get_user_pnl(env: Env, user: Address) -> i128 {
        env.storage()
            .persistent()
            .get(&DataKey::UserTotalPnL(user))
            .unwrap_or(0)
    }

    fn update_leaderboard(env: &Env, user: Address, total_pnl: i128) {
        let mut board: soroban_sdk::Vec<LeaderboardEntry> = env
            .storage()
            .persistent()
            .get(&DataKey::GlobalLeaderboard)
            .unwrap_or(soroban_sdk::Vec::new(env));
        ...
        env.storage()
            .persistent()
            .set(&DataKey::GlobalLeaderboard, &board);
        env.storage()
            .persistent()
            .extend_ttl(&DataKey::GlobalLeaderboard, 10_000, 100_000);
    }
    ...
    pub fn register_referral(env: Env, referee: Address, referrer: Address) -> Result<(), Error> {
        ...
        if env
            .storage()
            .persistent()
            .has(&DataKey::ReferralOf(referee.clone()))
        {
            return Ok(());
        }
        env.storage()
            .persistent()
            .set(&DataKey::ReferralOf(referee.clone()), &referrer);
        env.storage().persistent().extend_ttl(
            &DataKey::ReferralOf(referee.clone()),
            10_000,
            100_000,
        );
        ...
        env.storage().persistent().extend_ttl(
            &DataKey::ReferralCount(referrer.clone()),
            10_000,
            100_000,
        );

        env.events().publish(
            (Symbol::new(&env, "referral"), referrer.clone()),
            (referee.clone(),),
        );
        Ok(())
    }
    ...
    pub fn claim_referral_kickback(env: Env, referrer: Address) -> Result<(), Error> {
        ...
        let mut bal: i128 = env
            .storage()
            .persistent()
            .get(&DataKey::MarginBalance(referrer.clone()))
            .unwrap_or(0);
        bal += kickback;
        env.storage()
            .persistent()
            .set(&DataKey::MarginBalance(referrer.clone()), &bal);
        env.storage().persistent().extend_ttl(
            &DataKey::MarginBalance(referrer.clone()),
            10_000,
            100_000,
        );
        ...
        env.storage()
            .persistent()
            .set(&DataKey::ReferralKickback(referrer.clone()), &0i128);
        env.events().publish(
            (Symbol::new(&env, "kickback_claim"), referrer.clone()),
            (kickback,),
        );
        Ok(())
    }
